...
이러한 재귀신경망의 체인같은 성질은 재귀신경망이 시퀀스와 리스트 데이터에 밀접하게 관련되어 있다는 것을 의미한다. 즉, 시퀀스와 리스트 데이터에 적용할수 있는 신경망의 자연스러운 구조라는 것이다.

그리고 그들은 확실히 사용됩니다! 지난 몇 년 동안 음성 인식, 언어 모델링, 번역, 이미지 캡션 등 다양한 문제에 RNN을 적용하는 데 성공했습니다. 목록은 계속됩니다. Andrej Karpathy의 훌륭한 블로그 게시물 인 Reason Neural Networks의 비합리적인 효과에 대해 RNN과 함께 달성 할 수있는 놀라운 업적에 대한 논의를 떠날 것입니다. 그러나 그들은 정말로 정말로 훌륭하다.

이러한 성공의 핵심은 "LSTMs"를 사용하는 것인데, 이것은 매우 특별한 종류의 반복적 인 신경망으로 많은 작업에서 표준 버전보다 훨씬 효과적입니다. 반복적 인 신경망을 기반으로 한 거의 모든 흥미 진진한 결과는 이들과 함께 달성됩니다. 이 에세이가 탐구 할 LSTM입니다.
