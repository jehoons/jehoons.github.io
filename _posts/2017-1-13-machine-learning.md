...
때로는 현재의 작업을 처리하기 위해서 단지 최근의 정보만을 살펴봐야 할때도 있다. 예를 들어, 이전 단어를 기반으로 다음 단어를 예측하려고 시도하는 언어 모델을 생각해보십시오. 우리가 "구름이 하늘에있다"라는 마지막 단어를 예측하려고한다면, 우리는 더 이상의 내용을 필요로하지 않습니다. 다음 단어가 하늘이 될 것이라는 것은 꽤 분명합니다. 그러한 경우, 관련 정보와 필요로하는 장소 사이의 간격이 작은 경우 RNN은 과거 정보를 사용하는 법을 배울 수 있습니다.
